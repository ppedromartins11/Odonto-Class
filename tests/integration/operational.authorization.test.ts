...
async function identity(role: Role) { const suffix = randomUUID(); const candidate = { email: `operacional-${role}-${suffix}@example.com`, password: `Tmp-${randomUUID()}-A9!`, role }; const { data, error } = await service.auth.admin.createUser({ email: candidate.email, password: candidate.password, email_confirm: true, user_metadata: { nome: `Usuario operacional ficticio ${suffix}`, perfil: role, created_by: adminId } }); if (error || !data.user) throw error ?? new Error("Usuario ficticio ausente."); const result = { ...candidate, id: data.user.id }; users.push(result); return result; }
...
describe("bloco operacional: RLS, Storage, documentos, retornos e tarefas", () => {
  beforeAll(async () => {
    ...
    admin = client(); const { data: login, error: loginError } = await admin.auth.signInWithPassword({ email: required("SUPABASE_TEST_ADMIN_EMAIL"), password: required("SUPABASE_TEST_ADMIN_PASSWORD") }); if (loginError || !login.user) throw new Error(`Administrador de homologacao invalido (${loginError?.code ?? "sem_usuario"}).`); adminId = login.user.id;
  ...
  it("aplica RLS clinico por metadado e impede bypass por UUID/path", async () => {
    ...
    const administrative = await reception.rpc("create_patient_file_metadata", { p_paciente_id: patientId, p_storage_path: administrativePath, p_nome_original: "administrativo.pdf", p_mime_type: "application/pdf", p_tamanho_bytes: 5, p_categoria: "administrativo" }); expect(administrative.error).toBeNull(); auditIds.push((administrative.data as { id: string }).id);
    ...
    const clinical = await dentistA.rpc("create_patient_file_metadata", { p_paciente_id: patientId, p_storage_path: clinicalPath, p_nome_original: "clinico.pdf", p_mime_type: "application/pdf", p_tamanho_bytes: 5, p_categoria: "clinico" }); expect(clinical.error).toBeNull(); clinicalFileId = (clinical.data as { id: string }).id; auditIds.push(clinicalFileId);
  ...
  it("gera metadado de PDF privado, isola documento e nao audita conteudo", async () => {
    ...
    const created = await admin.rpc("create_document_metadata", { p_paciente_id: patientId, p_profissional_id: professionalA, p_tipo: "atestado", p_emitido_em: new Date().toISOString().slice(0, 10), p_periodo_inicio: null, p_periodo_fim: null, p_texto_adicional: "conteudo clinico ficticio que nao pode entrar em auditoria", p_storage_path: documentPath, p_nome_arquivo: "atestado.pdf", p_tamanho_bytes: 5 }); expect(created.error).toBeNull(); documentId = (created.data as { id: string }).id; auditIds.push(documentId);
    ...
    const { data: audit } = await admin.from("auditoria").select("dados").eq("entidade_id", documentId); expect(JSON.stringify(audit)).not.toContain("conteudo clinico ficticio");
  });
  ...
  it("vincula retorno ao agendamento e o conclui ao atendimento", async () => {
    ...
    const returned = await dentistA.rpc("create_return", { p_atendimento_id: attendanceId, p_data_prevista: new Date(Date.now() + 86400000).toISOString().slice(0, 10), p_observacao_administrativa: "retorno ficticio" }); expect(returned.error).toBeNull(); returnId = (returned.data as { id: string }).id; auditIds.push(returnId);
    ...
    expect((await dentistA.rpc("finalize_attendance", { p_atendimento_id: linkedAttendance, p_evolucao: "evolucao ficticia" })).error).toBeNull(); const status = await reception.from("retornos").select("status,agendamento_id").eq("id", returnId).single(); expect(status.data).toMatchObject({ status: "concluido", agendamento_id: appointmentId });
  });

  it("mantem regras de tarefa na RPC e auditoria sem valores sensiveis", async () => {
    const created = await admin.rpc("create_task", { p_titulo: "Tarefa operacional ficticia", p_descricao: "conteudo sensivel ficticio", p_prazo: null, p_responsavel_id: receptionIdentity.id, p_paciente_id: patientId, p_agendamento_id: null }); expect(created.error).toBeNull(); taskId = (created.data as { id: string }).id; auditIds.push(taskId);
    expect((await reception.rpc("update_task", { p_tarefa_id: taskId, p_titulo: "Tarefa editada", p_descricao: "conteudo alterado sensivel", p_prazo: null, p_responsavel_id: receptionIdentity.id, p_paciente_id: null, p_agendamento_id: null })).error).toBeNull();
    expect((await dentistB.rpc("update_task", { p_tarefa_id: taskId, p_titulo: "negado", p_descricao: null, p_prazo: null, p_responsavel_id: dentistBIdentity.id, p_paciente_id: null, p_agendamento_id: null })).error).not.toBeNull();
    expect((await admin.rpc("update_task", { p_tarefa_id: taskId, p_titulo: "invalida", p_descricao: null, p_prazo: null, p_responsavel_id: inactiveIdentity.id, p_paciente_id: null, p_agendamento_id: null })).error).not.toBeNull();
    expect((await reception.rpc("set_task_status", { p_tarefa_id: taskId, p_status: "concluida" })).error).toBeNull();
    expect((await admin.rpc("update_task", { p_tarefa_id: taskId, p_titulo: "tardia", p_descricao: null, p_prazo: null, p_responsavel_id: receptionIdentity.id, p_paciente_id: null, p_agendamento_id: null })).error).not.toBeNull();
    ...
    for (const target of [inactiveIdentity, orphanIdentity]) { const blocked = await signed(target); expect((await blocked.from("arquivos_paciente").select("id")).data).toEqual([]); expect((await blocked.rpc("create_task", { p_titulo: "negada", p_descricao: null, p_prazo: null, p_responsavel_id: receptionIdentity.id, p_paciente_id: null, p_agendamento_id: null })).error).not.toBeNull(); }
    const { data: audit } = await admin.from("auditoria").select("dados").eq("entidade_id", taskId); expect(JSON.stringify(audit)).not.toContain("conteudo sensivel"); expect(JSON.stringify(audit)).not.toContain("conteudo alterado");
  });
});
