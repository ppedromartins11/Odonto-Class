...
export default async function PatientPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: SearchParams }) {
  ...
  const recentHistory = [
    ...
    ...(isDentist ? procedureEntries.slice(0, 4).map((item) => ({ id: `procedure-${item.id}`, date: item.created_at, title: `Procedimento: ${item.descricao}`, description: `${item.attendance.profissional_nome}${item.teeth.length > 0 ? ` · Dentes ${item.teeth.join(", ")}` : ""}${item.dente ? ` · Dente/região ${item.dente}` : ""}${item.detalhes ? ` · ${item.detalhes}` : ""}` })) : []),
