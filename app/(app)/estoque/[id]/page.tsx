...
import { LotControlForm } from "../../validade/ValidityForms";
...
export default async function StockMaterialPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="mx-auto max-w-6xl space-y-5"><Link href="/estoque" className="text-sm text-muted-foreground hover:text-foreground">← Voltar para estoque</Link><header className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-2xl font-medium">{material.nome}</h2><p className="mt-1 text-sm text-muted-foreground">{material.categoria} · {unitLabels[material.unidade]}</p></div>{admin && <StockActiveForm materialId={material.id} active={material.ativo} />}</header>
    ...
    {admin && <LotControlForm materialId={material.id} enabled={Boolean(material.controla_lote_validade)} currentQuantity={material.quantidade_atual} />}
    {material.controla_lote_validade ? <section className="rounded-xl border border-border bg-card p-5"><h3 className="font-medium">Movimentação controlada por lote</h3><p className="mt-1 text-sm text-muted-foreground">Entradas e saídas deste material exigem lote, validade e rastreabilidade. O saldo agregado continua sincronizado na mesma transação.</p><Link href="/validade" className="mt-3 inline-flex text-sm font-medium text-primary hover:underline">Abrir validade e lotes</Link></section> : material.ativo && <section className="grid gap-4 lg:grid-cols-3"><StockMovementForm materialId={id} type="saida" profile={user.perfil} />{user.perfil !== "dentista" && <StockMovementForm materialId={id} type="entrada" profile={user.perfil} />}{admin && <StockMovementForm materialId={id} type="ajuste" profile={user.perfil} />}</section>}
