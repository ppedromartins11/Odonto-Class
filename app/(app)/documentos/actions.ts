...
export async function createDocument(_: DomainActionState, form: FormData): Promise<DomainActionState> {
  await requireUser(); const patientId=String(form.get("patientId")??form.get("pacienteId")??""); const professionalId=String(form.get("professionalId")??""); const type=String(form.get("type")??"") as DocumentType;
